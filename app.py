...
if selected_tab == "📊 Operational Dashboard":
    ...
    if not active_df.empty:
        ...
        # ACTIVE BATCHES EXPANDABLE DIRECTORY
        ...
        with col_al1:
            if not aging_lots.empty:
                lots_str = ", ".join([f"<b>{r['Lot No.']}</b> ({r['Days in Storage']}d)" for _, r in aging_lots.head(6).iterrows()])
                st.markdown(f"""
        ...
        with col_al2:
            if not low_stock_lots.empty:
                low_str = ", ".join([f"<b>{r['Lot No.']}</b> ({r['Bal. Units']} left)" for _, r in low_stock_lots.head(6).iterrows()])
                st.markdown(f"""
                <div class="alert-banner alert-info">
