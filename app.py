...
# ---------------------------------------------------------
# THEME PALETTES & DYNAMIC CSS
# ---------------------------------------------------------
THEMES = {
    "Classic Slate": {
        "accent": "#2563EB",
        "accent_border": "rgba(37, 99, 235, 0.45)",
        "gradient1": "rgba(37, 99, 235, 0.08)",
        "gradient2": "rgba(16, 185, 129, 0.07)",
        "badge_border": "#3B82F6",
        "icon": "🏢",
    },
    "Warm Spice (Lehri Brand)": {
        "accent": "#DC2626",
        "accent_border": "rgba(220, 38, 38, 0.45)",
        "gradient1": "rgba(234, 88, 12, 0.10)",
        "gradient2": "rgba(220, 38, 38, 0.07)",
        "badge_border": "#EA580C",
        "icon": "🌶️",
    },
    "Emerald Fresh": {
        "accent": "#059669",
        "accent_border": "rgba(5, 150, 105, 0.45)",
        "gradient1": "rgba(16, 185, 129, 0.10)",
        "gradient2": "rgba(13, 148, 136, 0.07)",
        "badge_border": "#10B981",
        "icon": "🌿",
    },
    "Midnight OLED": {
        "accent": "#06B6D4",
        "accent_border": "rgba(6, 182, 212, 0.45)",
        "gradient1": "rgba(6, 182, 212, 0.08)",
    ...
    "Nordic Frost": {
        "accent": "#4F46E5",
        "accent_border": "rgba(79, 70, 229, 0.45)",
        "gradient1": "rgba(99, 102, 241, 0.09)",
        "gradient2": "rgba(59, 130, 246, 0.07)",
        "badge_border": "#6366F1",
        "icon": "❄️",
    },
}
...
if "app_theme" not in st.session_state:
  st.session_state.app_theme = "Classic Slate"
...
st.markdown(
    f"""
    <style>
    /* Background Canvas & Dynamic Mesh Glow */
    .stApp {{
        background-color: var(--background-color);
        background-image: 
            radial-gradient(at 0% 0%, {active_palette['gradient1']} 0px, transparent 55%),
            radial-gradient(at 100% 0%, {active_palette['gradient2']} 0px, transparent 55%),
            radial-gradient(at 50% 100%, rgba(128, 128, 128, 0.03) 0px, transparent 50%);
    ...
    .live-calc-box {{
        ...
        border: 1.5px dashed {active_palette['accent_border']};
        border-radius: 12px;
        padding: 12px 16px;
        text-align: center;
        margin-top: 4px;
    ...
    /* Pill-Style Segmented Navigation Menu */
...
with col_h2:
  ...
  st.markdown(
      "<div class='sub-header'>Cloud-Enabled Batch Tracking & Inventory"
      " Ledger</div>",
      unsafe_allow_html=True,
  )
...
with st.sidebar:
  ...
  new_theme = st.selectbox(
      ...
      help="Select a color scheme that suits your display",
  )
...
# ---------------------------------------------------------
# APPLICATION NAVIGATION MENU (SEGMENTED PILLS)
# ---------------------------------------------------------
...
if selected_tab == "📊 Operational Dashboard":
  ...
  if not active_df.empty:
      ...
      st.markdown(
          f"""
            <div class="kpi-card">
                ...
                <div class="kpi-sub">Active in cold storage</div>
            </div>
            """,
          unsafe_allow_html=True,
      )
    ...
    # 3. COLD STORAGE FACILITIES DIRECTORY (LOCATED DIRECTLY BELOW ACTIVE BATCHES)
    fac_names = sorted(list(active_df["Cold Storage"].unique()))
    with st.expander(
        f"🏢 **Cold Storage Facilities ({len(fac_names)} Facilities)** — Click"
        " to Expand & Dispatch"
    ):
      for fac_name in fac_names:
        ...
        st.markdown(
            f"##### 🏬 **{fac_name}** — {tot_fac_u:,} Units | {tot_fac_kg:,.2f}"
            f" KG ({tot_fac_lots} Lots)"
        )
  ...
  with st.container(border=True):
    ...
    if st.button("Save Outward Entry", type="primary", use_container_width=True):
      ...
      if not (out_lot_clean and final_cs and final_item):
        st.error(
            "Please fill in all required fields (Lot No., Cold Storage Name,"
            " and Item Name)."
        )
      elif not re.match(r"^[0-9/-]+$", out_lot_clean):
        st.error(
            "Invalid Lot No.: Alphabets are not allowed. Use only numbers, '-',"
            " or '/'."
        )
      else:
  ...
  with st.container(border=True):
    ...
    if lookup_mode == "Search by Lot No.":
      ...
      if sel_in_item:
        ...
        sel_lot_final = (
            sel_lot_display.split(" ")[0] if sel_lot_display else ""
        )
    ...
    if sel_lot_final and sel_item_final:
      st.info(
          f"Selected Lot **{sel_lot_final}** has **{max_avail_units} units**"
          f" of **{sel_item_final}** remaining in storage."
      )
...
# =========================================================
  ...
  with st.container(border=True):
    ...
    sel_cs = rf6.selectbox(
        "Cold Storage Filter", options=all_cs, key="rep_cs_f"
    )
